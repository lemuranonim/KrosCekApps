...
class _UserSettingsScreenState extends ConsumerState<UserSettingsScreen> {
  ...
  Future<void> _logout() async {
    final confirmed = await _showConfirmDialog(
      ...
      message: 'Semua data sesi akan dihapus dari perangkat ini. Anda perlu login ulang.',
      confirmLabel: 'KELUAR',
      isDestructive: true,
    );
  ...
  Future<bool> _showConfirmDialog({
    ...
    final result = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text(title, style: AdvantaText.heading2.copyWith(color: theme.colorScheme.onSurface)),
        content: Text(message, style: AdvantaText.body1.copyWith(color: theme.colorScheme.onSurface.withAlpha(180))),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
            child: Text('Batal', style: AdvantaText.bodyBold.copyWith(color: AdvantaColors.mutedGrey)),
          ),
          ElevatedButton(
            style: isDestructive
                ? ElevatedButton.styleFrom(backgroundColor: AdvantaColors.error, foregroundColor: Colors.white)
                : null,
      ...
      body: _isLoading
        ...
        children: [
          ...
          _SettingsTile(
            ...
            onTap: () => _showFeatureUnderDevelopment('Hybrid & Varietas'),
          ),
          ...
          _SettingsTile(
            ...
            subtitle: 'Hapus cache lokal dan ambil data terbaru dari server',
          ...
          Padding(
            ...
            child: OutlinedButton.icon(
              icon: const Icon(Icons.logout_rounded, color: AdvantaColors.error),
              label: const Text('Keluar dari Akun', style: TextStyle(color: AdvantaColors.error)),
              style: OutlinedButton.styleFrom(
                side: const BorderSide(color: AdvantaColors.error, width: 1.2),
  ...
  Future<void> _forceSyncAll() async {
    final confirmed = await _showConfirmDialog(
      ...
      message: 'Semua cache lokal akan dihapus dan data akan diambil ulang dari server. Pastikan koneksi internet aktif.',
  ...
  Future<void> _showCacheInfo() async {
    ...
    final userKeys = prefs.getKeys().where((k) => k.startsWith('u_${uid}_')).toList();
            ...
            Center(
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: isDark ? AdvantaColors.goldLight.withAlpha(50) : AdvantaColors.mutedGrey.withAlpha(80),
                  borderRadius: BorderRadius.circular(2),
                ),
              ),
            ),
            Text('Status Cache', style: AdvantaText.heading2.copyWith(color: theme.colorScheme.onSurface)),
            const SizedBox(height: 16),
            _CacheInfoRow(label: 'User aktif', value: uid.isEmpty ? 'Tidak ada' : '${uid.substring(0, 8)}...'),
            _CacheInfoRow(label: 'Key milik user ini', value: '${userKeys.length} item'),
            _CacheInfoRow(label: 'Total key di perangkat', value: '$totalKeys item'),
  ...
  void _showAboutDialog() {
    showAboutDialog(
      ...
      applicationLegalese: '© 2024-${DateTime.now().year} Advanta Seeds Indonesia. All rights reserved.',
    );
  }
}
...
class _ProfileSheetContentState extends ConsumerState<_ProfileSheetContent> {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          _CacheInfoRow(label: 'Role', value: (widget.session?.role ?? '-').toUpperCase()),
          _CacheInfoRow(
            label: 'Akses',
          ...
          if (!widget.isGuest)
            renameAsync.when(
              ...
              data: (renameState) => _buildRenameSection(
                context, theme, isDark, renameState,
              ),
            )
          else
            _InfoBanner(
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          child: Row(
            children: [
              Icon(Icons.person_outline, size: 14, color: AdvantaColors.mutedGrey),
  ...
  Future<void> _doRename() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() { _showRenameForm = false; _isSaving = false; });
        widget.onRenameSuccess();
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('Nama berhasil diubah dan data wilayah telah diperbarui.'),
            backgroundColor: AdvantaColors.primaryGreen,
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
          ),
...
class _ProfileCard extends StatelessWidget {
  ...
  const _ProfileCard({required this.session, required this.isDark, required this.theme});
      ...
      child: Row(
        children: [
          CircleAvatar(
            ...
            child: Text(
              initials,
              style: AdvantaText.heading2.copyWith(color: Colors.white, fontSize: 20),
            ),
          ),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isDark ? AdvantaColors.goldLight.withAlpha(20) : AdvantaColors.charcoal.withAlpha(10),
        ),
      ),
      child: ListTile(
        ...
        title: Text(label, style: AdvantaText.bodyBold.copyWith(color: theme.colorScheme.onSurface)),
        subtitle: subtitle != null
            ? Text(subtitle!, style: AdvantaText.caption.copyWith(color: AdvantaColors.mutedGrey))
            : null,
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Text(
        ...
        style: TextStyle(color: fg, fontSize: 10, fontWeight: FontWeight.w800, letterSpacing: 1),
      ),
    );
  }
}
...
class _CacheInfoRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          Text('$label:', style: AdvantaText.body2.copyWith(color: AdvantaColors.mutedGrey)),
          const SizedBox(width: 8),
          Expanded(
            child: Text(
              value,
