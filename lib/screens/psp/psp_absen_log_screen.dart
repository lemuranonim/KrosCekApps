...
class PspAbsenLogScreen extends StatefulWidget {
  ...
  @override
  PspAbsenLogScreenState createState() =>
      PspAbsenLogScreenState(); // Mengubah ke public
}
...
class PspAbsenLogScreenState extends State<PspAbsenLogScreen> {
  ...
  final Color _primaryColor = const Color(0xFFD32F2F); // Warna merah utama
  final Color _primaryLightColor =
      const Color(0xFFFF6659); // Warna merah lebih terang
  final Color _primaryDarkColor =
      const Color(0xFF9A0007); // Warna merah lebih gelap
  final Color _backgroundColor = const Color(0xFFF5F5F6); // Tetap sama (netral)
  final Color _cardColor = Colors.white; // Tetap putih
  final Color _textColor = const Color(0xFF263238); // Tetap gelap untuk kontras
  ...
  Future<void> _loadSpreadsheetId() async {
    ...
    String? selectedRegion =
        prefs.getString('selectedRegion'); // Ambil region yang dipilih

    if (selectedRegion != null) {
      String? spreadsheetId = ConfigManager.getSpreadsheetId(
          selectedRegion); // Ambil ID dari ConfigManager
      setState(() {
    ...
    if (status.isGranted) {
      try {
        ...
        setState(() {
          ...
          _isLoading = false;
          _hasTakenAttendance =
              true; // Set flag to true after successful attendance
        });
      } catch (e) {
  ...
  Future<void> _submitData() async {
    ...
    if (_googleSheetsApi == null || _spreadsheetId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content:
              Text('Spreadsheet ID tidak ditemukan untuk region yang dipilih.'),
          backgroundColor: Colors.red,
          behavior: SnackBarBehavior.floating,
        ),
      );
          ...
          : () async {
              if (_spreadsheetId == null) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text(
                        'Harap pilih Region terlebih dahulu sebelum Absen!'),
                    backgroundColor: Colors.red,
                    behavior: SnackBarBehavior.floating,
                  ),
                );
