...
class HarvestMapView extends StatefulWidget {
  final List<List<String>> filteredData;
  final String? selectedRegion;
  final Map<String, int> activityCounts;

  const HarvestMapView({
    super.key,
  ...
  @override
  State<HarvestMapView> createState() => _HarvestMapViewState();
}

class _HarvestMapViewState extends State<HarvestMapView> with AutomaticKeepAliveClientMixin {
  ...
  @override
  void didUpdateWidget(HarvestMapView oldWidget) {
  ...
  void _processMarkersInBatches(List<List<String>> data) {
    ...
    for (var row in data) {
      ...
      if (coordinateStr.isNotEmpty) {
        if (_getValue(row, 43, "NOT Audited") == "Audited") {
          auditedCount++;
        } else {
          notAuditedCount++;
        }
  ...
  Marker? _createMarkerFromRow(List<String> row) {
    try {
      ...
      final isAudited = _getValue(row, 43, "NOT Audited") == "Audited";
