...
class VegetativeFilterOptions extends StatefulWidget {
  final String? selectedSeason;
  final List<String> seasonsList;
  final Function(String?) onSeasonChanged;

  final List<String> selectedWeekOfVegetative;
  final List<String> weekOfVegetativeList;
  final Function(List<String>) onWeekOfVegetativeChanged;
  ...
  const VegetativeFilterOptions({
    super.key,
    this.selectedSeason,
    required this.seasonsList,
    required this.onSeasonChanged,
    required this.selectedWeekOfVegetative,
    required this.weekOfVegetativeList,
    required this.onWeekOfVegetativeChanged,
  ...
  @override
  State<VegetativeFilterOptions> createState() => _VegetativeFilterOptionsState();
}

class _VegetativeFilterOptionsState extends State<VegetativeFilterOptions> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _selectedWeeks = List.from(widget.selectedWeekOfVegetative);
                  ...
                  Flexible(
                    child: Scrollbar(
                      ...
                      child: ListView.separated(
                        padding: const EdgeInsets.all(8),
                        shrinkWrap: true,
                        itemCount: widget.weekOfVegetativeList.length,
                        separatorBuilder: (context, index) => const Divider(height: 1),
                        itemBuilder: (context, index) {
                          final item = widget.weekOfVegetativeList[index];
                    ...
                    child: Row(
                      ...
                      children: [
                        TextButton.icon(
                          onPressed: () {
                            setState(() {
                              _selectedWeeks = List.from(widget.weekOfVegetativeList);
                            });
                          },
  ...
  Widget _buildFAFilter() {
    return Container(
      ...
      child: Theme(
        ...
        child: ExpansionTile(
          initiallyExpanded: false, // Ensure it's not initially expanded
          tilePadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
          title: Row(
            children: [
              Icon(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return CheckboxListTile(
                            title: Text(
                              ...
                              style: TextStyle(
                                fontSize: 14,
                                fontWeight: isSelected ? FontWeight.bold : FontWeight.normal, color: isSelected ? Colors.amber.shade700 : Colors.black87,
                              ),
                            ),
                            value: isSelected,
                            activeColor: Colors.amber.shade600,
          ...
          Expanded(
            ...
            child: ElevatedButton(
              onPressed: () {
                ...
                widget.onWeekOfVegetativeChanged(_selectedWeeks);
                widget.onFAChanged(_selectedFA);
                widget.onFIChanged(_selectedFIs);
                widget.onShowDiscardedFaseChanged(_localShowDiscardedFase);
