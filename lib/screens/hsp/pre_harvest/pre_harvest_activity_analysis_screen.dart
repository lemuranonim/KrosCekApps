...
class PreHarvestActivityAnalysisScreen extends StatefulWidget {
  final Map<String, int> activityCounts;
  final Map<String, List<DateTime>> activityTimestamps;
  final List<List<String>> preHarvestData;
  final String? selectedRegion;

  const PreHarvestActivityAnalysisScreen({
    super.key,
    required this.activityCounts,
    required this.activityTimestamps,
    required this.preHarvestData,
    this.selectedRegion,
  });

  @override
  State<PreHarvestActivityAnalysisScreen> createState() => _PreHarvestActivityAnalysisScreenState();
}

class _PreHarvestActivityAnalysisScreenState extends State<PreHarvestActivityAnalysisScreen> with SingleTickerProviderStateMixin {
  ...
  void _calculateStatistics() {
    // Basic counts
    _totalFields = widget.preHarvestData.length;
    ...
    for (var row in widget.preHarvestData) {
  ...
  List<List<String>> _getFilteredData() {
    return widget.preHarvestData.where((row) {
  ...
  List<List<String>> _getSortedData(List<List<String>> filteredData) {
    ...
    switch (_sortColumn) {
      ...
      case 'auditStatus':
        sortedData.sort((a, b) {
          final isAuditedA = _getValue(a, 55, "NOT Audited") == "Audited";
          final isAuditedB = _getValue(b, 55, "NOT Audited") == "Audited";
          return _sortAscending
              ? isAuditedA.toString().compareTo(isAuditedB.toString())
              : isAuditedB.toString().compareTo(isAuditedA.toString());
        });
        break;
      case 'dap':
        sortedData.sort((a, b) {
            ...
            Padding(
              ...
              child: Center(
                child: TextButton.icon(
                  ...
                  onPressed: () {
                    _tabController.animateTo(3); // Switch to Data Table tab
                  },
                  style: TextButton.styleFrom(
                ...
                child: Row(
                  children: [
                    FilterChip(
                      ...
                      onSelected: (selected) {
                        setState(() {
                          _showAuditedOnly = selected;
                          if (selected) _showNotAuditedOnly = false;
                        });
                      },
                      selectedColor: AppTheme.success.withAlpha(51),
                      checkmarkColor: AppTheme.success,
                      avatar: _showAuditedOnly
                          ? const Icon(Icons.check_circle, size: 18, color: AppTheme.success)
                          : null,
                    ),
                    const SizedBox(width: 8),
                    FilterChip(
                      label: const Text('Not Audited Only'),
                      selected: _showNotAuditedOnly,
                      onSelected: (selected) {
                        setState(() {
                          _showNotAuditedOnly = selected;
                          if (selected) _showAuditedOnly = false;
                        });
                      },
                      selectedColor: AppTheme.error.withAlpha(51),
                      checkmarkColor: AppTheme.error,
                      avatar: _showNotAuditedOnly
                    ...
                    FilterChip(
                      ...
                      selected: false, // Implement this filter if needed
                      onSelected: (selected) {
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: const [
              ...
              SizedBox(height: 4),
              Text(
                'Shows summary statistics and key metrics about Pre-Harvest activity. The cards display information about activity status, audit status, and area analysis.',
              ),
              SizedBox(height: 12),
              ...
              SizedBox(height: 4),
              Text(
                'Shows detailed information for each field in a sortable table. Click on a field number to view more details about that specific field.',
              ),
              SizedBox(height: 12),
