...
class PreHarvestMapView extends StatefulWidget {
  final List<List<String>> filteredData;
  final String? selectedRegion;
  final Map<String, int> activityCounts;

  const PreHarvestMapView({
    super.key,
  ...
  @override
  State<PreHarvestMapView> createState() => _PreHarvestMapViewState();
}

class _PreHarvestMapViewState extends State<PreHarvestMapView> with AutomaticKeepAliveClientMixin {
  ...
  @override
  void didUpdateWidget(PreHarvestMapView oldWidget) {
  ...
  void _processMarkersInBatches(List<List<String>> data) {
    ...
    for (var row in data) {
      ...
      if (coordinateStr.isNotEmpty) {
        if (_getValue(row, 39, "NOT Audited") == "Audited") {
          auditedCount++;
        } else {
          notAuditedCount++;
        }
