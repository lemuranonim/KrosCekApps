...
class _GDUMonitoringPageState extends State<GDUMonitoringPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      _phaseProgress = _service.getPhaseProgress(_gdu);
  ...
  void _calculateAdditionalMetrics() {
    ...
    if (avgGDUPerDay >= idealGDUPerDay * 1.1) {
      _weatherImpact = {
        ...
        'percentage': ((avgGDUPerDay / idealGDUPerDay - 1) * 100).clamp(0, 100),
      };
      ...
      _weatherImpact = {
        ...
        'percentage': ((avgGDUPerDay / idealGDUPerDay) * 100).clamp(0, 100),
      };
    } else {
      _weatherImpact = {
...
class _PremiumGDUScreenState extends State<PremiumGDUScreen> with TickerProviderStateMixin {
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: Icon(Icons.warning_amber_rounded, color: Colors.orange[300], size: 24),
                ),
                SizedBox(width: 14),
                Expanded(child: Text(widget.alert!, style: TextStyle(color: Colors.orange[100], fontSize: 13, fontWeight: FontWeight.w500, height: 1.4))),
              ],
            ),
          ),
        );
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: 2),
                        Text(widget.gduStatus['mainPhase'], style: TextStyle(color: Colors.white.withAlpha((0.5 * 255).toInt()), fontSize: 12, letterSpacing: 0.5)),
                      ],
                    ),
                  ),
                  Container(
                        ...
                        children: [
                          Row(
                            children: [
                              ...
                              SizedBox(width: 6),
                              Text('Estimasi Hari', style: TextStyle(color: Colors.white.withAlpha((0.6 * 255).toInt()), fontSize: 12)),
                            ],
                          ),
                          SizedBox(height: 8),
                          Text(widget.gduStatus['expectedDays'], style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ),
                  ],
              ...
              Column(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      Text('Progress → ${widget.gduStatus['nextMilestone']}', style: TextStyle(color: Colors.white.withAlpha((0.8 * 255).toInt()), fontSize: 13, fontWeight: FontWeight.w500)),
                      Container(
                  ...
                  Row(
                    children: [
                      ...
                      SizedBox(width: 6),
                      Text('${widget.gduStatus['gduToNext']?.toStringAsFixed(0)} GDU lagi', style: TextStyle(color: Colors.white.withAlpha((0.5 * 255).toInt()), fontSize: 12)),
                    ],
                  ),
                ],
              ),
          ...
          Row(
            children: [
              Container(
                ...
                child: Icon(Icons.insights, color: Colors.indigo[200], size: 24),
              ),
              SizedBox(width: 14),
              Expanded(
                child: Column(
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(insight['title'], style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w600)),
                        SizedBox(height: 4),
                        Text(insight['value'], style: TextStyle(color: insight['color'], fontSize: 12, fontWeight: FontWeight.bold)),
                      ],
                    ),
                  ),
                ],
  ...
  List<Map<String, dynamic>> _getPhaseInsights() {
    ...
    final nextPhaseGDU = _getNextPhaseGDU();
    final daysToNext = nextPhaseGDU > 0 && avgGDUPerDay > 0 ? (nextPhaseGDU / avgGDUPerDay).ceil() : 0;
    ...
    return [
      ...
      },
      {
        ...
        'title': 'Estimasi ke Fase Berikutnya',
        'value': '$daysToNext hari lagi',
      },
      {
  ...
  double _getNextPhaseGDU() {
    final phases = [263.9, 483.3, 777.8, 1069.4, 1500.0];
    for (var phase in phases) {
      if (widget.gdu < phase) return phase - widget.gdu;
    }
    return 0;
  }
  ...
  String _getGrowthStatus() {
    final avgGDUPerDay = widget.dap > 0 ? widget.gdu / widget.dap : 0;
    if (avgGDUPerDay >= 15) return 'Optimal';
    if (avgGDUPerDay >= 12) return 'Baik';
  ...
  Widget _buildWeatherImpactCard() {
    ...
    final percentage = impact['percentage'] as double;
          ...
          Row(
            children: [
              Container(
                ...
                child: Icon(impact['icon'], color: color, size: 24),
              ),
              SizedBox(width: 14),
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(impact['status'], style: TextStyle(color: color, fontSize: 13, fontWeight: FontWeight.w600)),
                  ],
                ),
              ),
              Container(
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                SizedBox(width: 10),
                Expanded(
                  child: Text(impact['description'], style: TextStyle(color: Colors.white.withAlpha((0.7 * 255).toInt()), fontSize: 13, height: 1.4)),
                ),
              ],
            ),
          ),
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 8),
                      Text(idealVelocity.toStringAsFixed(0), style: TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.bold)),
                      Text('GDU/hari', style: TextStyle(color: Colors.white60, fontSize: 11)),
                    ],
                  ),
                ),
  ...
  Widget _buildPhaseTimeline() {
    final phases = [
      {'phase': 'PL', 'gdu': 0.0, 'label': 'Tanam'},
      {'phase': 'V6', 'gdu': 263.9, 'label': 'V6'},
      {'phase': 'V12', 'gdu': 483.3, 'label': 'V12'},
      {'phase': 'VT', 'gdu': 777.8, 'label': 'VT'},
      {'phase': 'R4', 'gdu': 1069.4, 'label': 'R4'},
      {'phase': 'R6', 'gdu': 1500.0, 'label': 'Panen'},
    ];
        ...
        children: [
          Row(
            children: [
              ...
              SizedBox(width: 10),
              Text('Timeline Fase Pertumbuhan', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
            ],
          ),
          SizedBox(height: 24),
          SizedBox(
            ...
            child: CustomPaint(
              painter: TimelinePainter(
                ...
                currentPhase: widget.gduStatus['phase'],
              ),
              child: Container(),
            ),
          ),
        ...
        children: [
          Row(
            children: [
              ...
              SizedBox(width: 10),
              Text('Milestone Berikutnya', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
            ],
          ),
          SizedBox(height: 20),
              ...
              child: Row(
                children: [
                  ...
                  Column(
                    ...
                    children: [
                      Text(
                        milestone['reached'] ? '✓ Selesai' : '${milestone['days']} hari',
                        style: TextStyle(
                      ...
                      Text('${milestone['gdu'].toStringAsFixed(0)} GDU', style: TextStyle(color: Colors.white60, fontSize: 11)),
                    ],
                  ),
                ],
              ),
  ...
  List<Map<String, dynamic>> _getMilestones() {
    final avgGDUPerDay = widget.dap > 0 ? widget.gdu / widget.dap : 15.0;

    final phases = [
      {'name': 'Fase V6', 'gdu': 263.9, 'description': '6 daun sejati'},
      {'name': 'Fase V12', 'gdu': 483.3, 'description': '12 daun sejati'},
      {'name': 'Fase VT', 'gdu': 777.8, 'description': 'Tasseling'},
      {'name': 'Fase R4', 'gdu': 1069.4, 'description': 'Dough stage'},
      {'name': 'Panen', 'gdu': 1500.0, 'description': 'Matang panen'},
    ];

    return phases.map((phase) {
      final gdu = phase['gdu'] as double;
      final reached = widget.gdu >= gdu;
      final remaining = gdu - widget.gdu;
      final days = reached ? 0 : (remaining / avgGDUPerDay).ceil();
  ...
  Widget _buildHarvestCard() {
    if (widget.estimatedHarvestDate == null) {
      return Container(
        ...
        child: Center(
          child: Text('Menghitung Estimasi Panen...', style: TextStyle(color: Colors.white54)),
        ),
      );
    }
    ...
    final String formattedDate = DateFormat('dd MMMM yyyy').format(widget.estimatedHarvestDate!);
    ...
    if (hasHarvestPassed) {
      ...
      borderColor = Colors.brown.withAlpha((0.4 * 255).toInt());
      boxShadowColor = Colors.brown.withAlpha((0.2 * 255).toInt());
    } else {
      ...
      borderColor = Colors.amber.withAlpha((0.4 * 255).toInt());
      boxShadowColor = Colors.amber.withAlpha((0.2 * 255).toInt());

      if (daysDifference == 0) {
        subtitle = '🎉 Panen Hari Ini!';
      } else if (daysDifference == 1) {
        subtitle = '⏰ Besok!';
      } else {
        subtitle = '📅 Dalam $daysDifference hari lagi';
      }
    }
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: boxShadowColor, blurRadius: 16, offset: Offset(0, 8))],
      ),
      child: Row(
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                colors: [gradientColors[0].withAlpha((0.4 * 255).toInt()), gradientColors[1].withAlpha((0.3 * 255).toInt())],
              ),
              borderRadius: BorderRadius.circular(16),
            ),
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(title, style: TextStyle(color: Colors.white.withAlpha((0.7 * 255).toInt()), fontSize: 13)),
                SizedBox(height: 6),
                Text(formattedDate, style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
                SizedBox(height: 4),
                Text(subtitle, style: TextStyle(color: iconColor.withAlpha((0.9 * 255).toInt()), fontSize: 13, fontWeight: FontWeight.w500)),
              ],
            ),
          ),
        ],
  ...
  List<Map<String, dynamic>> _getRecommendations() {
    final phase = widget.gduStatus['phase'] as String;

    if (phase == 'PL' || phase == 'V2') {
      return [
        {
    ...
    return [
      {
        ...
        'description': 'Lakukan pemantauan pertumbuhan setiap minggu',
      },
    ];
  }
}
...
class TimelinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Progress line
    final progressWidth = (currentGDU / maxGDU * size.width).clamp(0.0, size.width);
    ...
    for (var phase in phases) {
      final gdu = phase['gdu'] as double;
      final x = (gdu / maxGDU * size.width).clamp(0.0, size.width);
      final isActive = gdu <= currentGDU;
      ...
      // GDU value for current
      if (isCurrent) {
        final gduTextPainter = TextPainter(
          text: TextSpan(
            text: '${currentGDU.toStringAsFixed(0)} °C',
            style: TextStyle(
        ...
        gduTextPainter.layout();
        gduTextPainter.paint(
          canvas,
          Offset(x - gduTextPainter.width / 2, size.height / 2 - 22),
        );
      }
    }
  }
...
// Custom Comparison Chart Painter
