...
class PspPreHarvestDetailScreen extends StatefulWidget {
  final String fieldNumber;
  final String region;

  const PspPreHarvestDetailScreen({
    super.key,
  ...
  @override
  PspPreHarvestDetailScreenState createState() => PspPreHarvestDetailScreenState();
}

class PspPreHarvestDetailScreenState extends State<PspPreHarvestDetailScreen> {
  late final GoogleSheetsApi _googleSheetsApi;
  bool _isLoading = true;
  String? _errorMessage;
  List<String>? _fieldData;

  // --- THEME COLORS: PRE-HARVEST (Red / Maroon / Brown) ---
  final Color _primaryColor = Colors.red.shade900;
  final Color _bgGradientTop = const Color(0xFFFFEBEE); // Red 50
  ...
  Future<void> _fetchFieldDetails() async {
    ...
    try {
      ...
      // TARGET SHEET: TETAP GENERATIVE (Karena data Pre-Harvest ada di sheet ini)
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: PreferredSize(
        preferredSize: const Size.fromHeight(0), // No standard AppBar
        child: AppBar(elevation: 0, backgroundColor: Colors.transparent),
      ),
      body: Stack(
        children: [
          // 1. Background Gradient (Red Tones)
          Container(
          ...
          // 2. Decorative Blob (Top Left - Red)
          Positioned(
            top: -60,
            left: -60,
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [
                  BoxShadow(color: Colors.red.shade300.withOpacity(0.2), blurRadius: 80, spreadRadius: 20),
                ],
              ),
            ),
          ),
                ...
                Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      const SizedBox(width: 44), // Placeholder for centering
                    ],
                  ),
                ),
  ...
  // --- WIDGET: Main Header Card (Red/Maroon Theme) ---
  Widget _buildMainHeaderCard() {
    ...
    // Index 12 (Kolom M) for Planting Date
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [_primaryColor, Colors.red.shade700],
      ...
      child: Stack(
        children: [
          // Background Pattern (Grain/Agriculture)
          Positioned(
            right: -20,
            top: -20,
            child: Icon(Icons.grain_rounded, size: 150, color: Colors.white.withOpacity(0.1)),
          ),
  ...
  List<Widget> _buildLocationList() {
    ...
    return [
      _buildInfoTile("Village (Desa)", desa, Icons.home_work_rounded, Colors.red.shade800),
      ...
      Divider(height: 1, indent: 60, color: Colors.grey.withOpacity(0.1)),
      _buildInfoTile("Regency (Kabupaten)", kab, Icons.map_rounded, Colors.orange.shade900),
      Divider(height: 1, indent: 60, color: Colors.grey.withOpacity(0.1)),
      _buildInfoTile("Zone Area", zone, Icons.flag_rounded, Colors.redAccent.shade700),
    ];
  }
  ...
  List<Widget> _buildPersonnelList() {
    ...
    return [
      _buildInfoTile("Field Assistant (FA)", fa, Icons.badge_rounded, Colors.brown),
    ];
  }
