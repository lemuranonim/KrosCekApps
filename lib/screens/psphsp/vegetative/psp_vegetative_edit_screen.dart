...
class PspVegetativeEditScreen extends StatefulWidget {
  final List<String> row;
  final String region;
  final Function(List<String>)
      onSave; // Callback untuk mengirim data yang diperbarui

  const PspVegetativeEditScreen(
      {super.key,
      required this.row,
  ...
  @override
  PspVegetativeEditScreenState createState() => PspVegetativeEditScreenState();
}

class PspVegetativeEditScreenState extends State<PspVegetativeEditScreen> {
  ...
  void initState() {
    ...
    _dateAuditController =
        TextEditingController(text: _convertToDateIfNecessary(row[33]));
    _actualPlantingDateController =
        TextEditingController(text: _convertToDateIfNecessary(row[35]));
  ...
  void _initHive() async {
    ...
    await Hive.openBox(
        'pspVegetativeData'); // Buat box Hive untuk menyimpan data vegetative
  }
            ...
            child: Column(
              children: [
                ...
                _buildDatePickerField(
                    'Date of Audit', 33, _dateAuditController),
                _buildDatePickerField('Actual Female Planting Date', 35,
                    _actualPlantingDateController),
  ...
  void _showLoadingAndSaveInBackground() {
    ...
    _saveToGoogleSheets(
        row); // Panggil fungsi penyimpanan yang berjalan di background
  }
...
class PspSuccessScreen extends StatelessWidget {
  ...
  Future<void> _saveBackActivityToGoogleSheets(String region) async {
    ...
    final String fieldNumber = row[3];
    final String regions = row[26];
