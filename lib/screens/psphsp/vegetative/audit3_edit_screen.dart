...
class Audit3EditScreen extends StatefulWidget {
  ...
  @override
  Audit3EditScreenState createState() => Audit3EditScreenState();
}

class Audit3EditScreenState extends State<Audit3EditScreen> {
  ...
  void initState() {
    ...
    _dateAuditController = TextEditingController(text: _convertToDateIfNecessary(row[53]));

    gSheetsApi = GoogleSheetsApi(spreadsheetId);
    gSheetsApi.init();

    // Initialize dropdown fields, set to null if empty
    selectedCropHealth = row[58].isNotEmpty ? row[58] : null;
    selectedCropUniformity = row[59].isNotEmpty ? row[59] : null;
  }
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildDatePickerField('Date of Audit 3', 53, _dateAuditController),
                        const SizedBox(height: 10),
                        _buildTextFormField('Audit 3 Offtype', 55, icon: Icons.text_fields),
                        const SizedBox(height: 10),
                        _buildTextFormField('Audit 3 Volunteer', 56, icon: Icons.group),
                        const SizedBox(height: 10),
                        _buildTextFormField('Audit 3 LSV', 57, icon: Icons.check_circle),
