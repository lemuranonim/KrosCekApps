...
class PspVisit5Screen extends StatefulWidget {
  final String spreadsheetId;
  final String fieldNumber;
  final String region;

  const PspVisit5Screen({
    super.key,
  ...
  @override
  State<PspVisit5Screen> createState() => _PspVisit5ScreenState();
}

class _PspVisit5ScreenState extends State<PspVisit5Screen> {
  ...
  Future<void> _submitData() async {
    ...
    try {
      // MAPPING KOLOM VISIT 5 (CQ, CT - DI)
      // CQ=95, CT=98 ... DI=113
      Map<int, String> updates = {
        95: _visitDateController.text,          // CQ: Date Of Visit 5
        // CR(96), CS(97) Skipped
        98: _roguingDateController.text,        // CT: Actual Roguing 2 Date
        99: _rogOfftypeController.text,         // CU: Roguing 2 Offtype
        100: _rogVolunteerController.text,      // CV: Roguing 2 Volunteer
        101: _rogLsvController.text,            // CW: Roguing 2 LSV
        102: _rogQtyController.text,            // CX: Qty Roguing 2 (Total)
        103: _selectedCropRate ?? "-",          // CY: Veg Crop Rate
        104: _selectedUniformity ?? "-",        // CZ: Veg Crop Uniformity
        105: _selectedHealth ?? "-",            // DA: Veg Crop Health
        106: _plantPerfRemarksController.text,  // DB: Plant Performance Remarks
        107: _envRemarksController.text,        // DC: Environment Remarks
        108: _selectedFertilizer3 ?? "-",       // DD: 3rd Fertilizer
        109: _selectedInsecticide ?? "-",       // DE: Spray Insecticide
        110: _selectedFungicide ?? "-",         // DF: Spray Fungicide
        111: _selectedFoliar ?? "-",            // DG: Spray Foliar
        112: _selectedRecommendation ?? "-",    // DH: Recommendation
        113: _recPldAreaController.text,        // DI: Recommendation PLD (Ha)
      };

      await _googleSheetsApi.updateSpecificCells('Vegetative', _targetRowIndex!, updates);

      if (mounted) {
        _showSnackBar("Visit 5 Data Saved Successfully!", isError: false);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Visit 5 Checklist", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    // --- SECTION 1: ROGUING ---
                    _buildSectionHeader("Roguing 2 & Schedule", Icons.group_remove_rounded),
                    ...
                    _buildGlassContainer(
                      children: [
                        GestureDetector(
                          ...
                          child: AbsorbPointer(
                            child: _buildTextField(_visitDateController, "Date of Visit 5", Icons.calendar_today),
                          ),
                        ),
                        const SizedBox(height: 10),
                        ...
                        Container(
                          ...
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text("Roguing 2 Details", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.orange.shade900)),
                    ...
                    _buildSectionHeader("Maintenance (Round 3)", Icons.build_circle_outlined),
                    const SizedBox(height: 10),
                    _buildGlassContainer(
                      children: [
                        _buildDropdownSimple("3rd Fertilizer", _statusOptions, (v) => _selectedFertilizer3 = v, value: _selectedFertilizer3, icon: Icons.science),
                        const SizedBox(height: 10),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        ...
                        child: _isSubmitting
                            ...
                            : const Text("SAVE VISIT 5", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
                      ),
                    ),
                  ],
                ),
