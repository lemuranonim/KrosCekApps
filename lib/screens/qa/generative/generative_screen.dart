...
import '../mixins/phase_screen_mixin.dart';
...
class GenerativeScreenState extends State<GenerativeScreen>
    with PhaseScreenInitializationMixin {

  // ✅ Implement required getters from mixin
  @override
  String get spreadsheetIdFromWidget => widget.spreadsheetId;

  @override
  String? get regionFromWidget => widget.region;

  // ❌ HAPUS INI - sudah ada di mixin sebagai `googleSheetsApi`
  // late final GoogleSheetsApi _googleSheetsApi;

  // ✅ GUNAKAN getter untuk akses GoogleSheetsApi dari mixin
  GoogleSheetsApi get _googleSheetsApi => googleSheetsApi;
  ...
  void initState() {
    ...
    // ✅ HANYA INI - semua logic ada di _initialize()
    _initialize();
  }

  // ✅ SIMPLIFIED - hanya satu method inisialisasi
  Future<void> _initialize() async {
    ...
    try {
      // ✅ Initialize from mixin (handles ConfigManager + GoogleSheetsApi)
      await initializePhaseScreen();
      ...
      setState(() {
        _errorMessage = "Gagal menginisialisasi: $e";
        _isLoading = false;
      });
    }
  }

  // ❌ HAPUS METHOD INI - sudah ditangani oleh mixin
  // Future<void> _ensureConfigManagerInitialized() async { ... }

  // ❌ HAPUS METHOD INI - sudah ditangani oleh mixin
  // void _initializeGoogleSheets() { ... }
  ...
  Widget build(BuildContext context) {
    // ✅ TAMPILKAN LOADING/ERROR dari mixin
    if (isInitializing) {
      return buildInitializingWidget();
    }
                ...
                _buildSliverCenteredContent(child: Lottie.asset('assets/loading.json'))
                  ...
                  _buildSliverCenteredContent(child: _buildErrorState())
                    ...
                    GenerativeSliverListBuilder(
                      ...
                      selectedRegion: selectedRegion,
                      activityCounts: _activityCounts,
                      selectedQA: widget.selectedQA, // ✅ Pass parameter
                      selectedSeason: widget.selectedSeason, // ✅ Pass parameter
                      seasonList: widget.seasonList, // ✅ Pass parameter
