...
void main() {
      ...
      httpClient: MockClient((request) async {
        ...
        return http.Response.bytes(
          utf8.encode(jsonEncode([
            {'field_number': 'F1', 'geometry_wkt': wkt}
          ])),
          200,
  ...
  test('coverage pages load concurrently and remain in field order', () async {
    ...
    final rows =
        await SupabaseService(client: client).getMasterFieldsForCoverage(
      region: 'Region 5',
      season: 'S1',
      qaFi: 'FI A',
    );
  ...
  test('large coverage status lists can be parsed off the UI isolate',
      () async {
    ...
    final container = ProviderContainer(overrides: [
      masterFieldCoverageScopedProvider
          .overrideWith((ref, scope) async => rawFields),
    ]);
    addTearDown(container.dispose);

    final statuses = await container.read(coverageStatusListScopedProvider(
