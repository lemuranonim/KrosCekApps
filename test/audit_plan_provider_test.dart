...
http.Response response(List<Map<String, dynamic>> rows) => http.Response(
  jsonEncode(rows),
  200,
...
ProviderContainer planningContainer(
...
Future<List<AuditPlanField>> loadPlan(
...
List<String> requestedNumbers(http.Request request) => request
    .url
    .queryParameters['field_number']!
    .replaceFirst('in.(', '')
...
void main() {
  test(
    'planning index is slim, scoped in the query and reads every page',
    () async {
      ...
      final service = mockService((request) async {
        ...
        if (offset > 0) {
          activePages++;
          if (activePages > maxActivePages) maxActivePages = activePages;
  ...
  test(
    'detail queries use bounded field-number batches and preserve scope',
    () async {
      final requests = <http.Request>[];
      var activeChunks = 0;
      ...
      final service = mockService((request) async {
        requests.add(request);
        activeChunks++;
        if (activeChunks > maxActiveChunks) maxActiveChunks = activeChunks;
        await Future<void>.delayed(const Duration(milliseconds: 20));
        activeChunks--;
        return response(
          requestedNumbers(request)
      ...
      for (final request in requests) {
        ...
        expect(request.url.queryParameters['select'], contains('farmer_name'));
        expect(
          request.url.queryParameters['select'],
          contains('effective_area_ha'),
        );
        expect(request.url.queryParameters['select'], contains('coordinate'));
        expect(
          request.url.queryParameters['select'],
          contains('flagging_final'),
        );
        expect(
          request.url.queryParameters['select'],
          isNot(contains('harvested_qty_kg')),
        );
        expect(
          request.url.queryParameters['select'],
          isNot(contains('previous_crop_data_a_b')),
        );
      }
    },
  );
  ...
  test(
    'a stalled page ends with timeout instead of a partial planning total',
    () async {
  ...
  test('details are requested only for weekly targets including revised dates '
    ...
    final rows = [
      ...
      fixtures.fieldAt(
        20,
        id: 'done-before-week',
        veg: {'date_of_audit': '2026-08-23'},
      ),
      fixtures.fieldAt(
        200,
        id: 'revised',
        veg: {'rev_planting_date': '04/08/2026'},
      ),
      fixtures.fieldAt(
        35,
        id: 'psp-partial',
        hybrid: 'ASF 1',
        veg: {'date_of_inspeksi_roguing_1': '2026-08-20'},
      ),
      fixtures.fieldAt(
        35,
        id: 'psp-done',
    ...
    expect(
      fields.map((field) => field.weekly.raw['field_number']),
  ...
  test(
    'changing week reuses the index and only reloads targeted details',
    () async {
      var indexRequests = 0;
  ...
  for (final role in ['FI', 'SPV']) {
    test(
      '$role keeps exact team scoping for both index and detail requests',
      () async {
        final requests = <http.Request>[];
        final service = mockService((request) async {
          requests.add(request);
        ...
        for (final request in requests) {
          expect(
            request.url.queryParameters['qa_${role.toLowerCase()}'],
            'ilike.%$role 1%',
          );
        }
      },
  ...
  test('a query error surfaces without automatic background retries', () async {
    ...
    final service = mockService((request) async {
      ...
      return http.Response(
        jsonEncode({'code': '42703', 'message': 'Column missing'}),
        400,
        headers: {'content-type': 'application/json'},
      );
    });
